...
int main(int argc, char **argv)
{
	...
	dim3 grid((size + blocksize - 1) / blockIdx.x, 1);
	...
	CHECK(cudaMemcpy(d_idata, h_idata, bytes, cudaMemcpyHostToDevice));
	reduceSmemDyn<<<grid.x, block>>>(d_idata, d_odata, size);
	CHECK(cudaMemcpy(h_odata, d_odata, grid.x * sizeof(int), cudaMemcpyDeviceToHost));
	gpu_sum = 0;
	for(int i = 0; i < grid.x, i++)
		gpu_sum += h_odata[i];
	...
	CHECK(cudaMemcpy(d_idata, h_idata, bytes, cudaMemcpyHostToDevice));
	reduceGmemUnroll<<<grid.x, block>>>(d_idata, d_odata, size);
	CHECK(cudaMemcpy(h_odata, d_odata, grid.x * sizeof(int), cudaMemcpyDeviceToHost));
	gpu_sum = 0;
	for(int i = 0; i < grid.x, i++)
		gpu_sum += h_odata[i];
	...
	CHECK(cudaMemcpy(d_idata, h_idata, bytes, cudaMemcpyHostToDevice));
	reduceSmemUnroll<<<grid.x, block>>>(d_idata, d_odata, size);
	CHECK(cudaMemcpy(h_odata, d_odata, grid.x * sizeof(int), cudaMemcpyDeviceToHost));
	gpu_sum = 0;
	for(int i = 0; i < grid.x, i++)
		gpu_sum += h_odata[i];
	...
	CHECK(cudaMemcpy(d_idata, h_idata, bytes, cudaMemcpyHostToDevice));
	reduceSmemUnrollDyn<<<grid.x, block>>>(d_idata, d_odata, size);
	CHECK(cudaMemcpy(h_odata, d_odata, grid.x * sizeof(int), cudaMemcpyDeviceToHost));
	gpu_sum = 0;
	for(int i = 0; i < grid.x, i++)
		gpu_sum += h_odata[i];
	printf("reduceSmemUnroll4Dyn: %d <<<grid %d block %d>>>>\n", gpu_sum, grid.x, block.x);
